            ...
            <div class="tab-content">
                ...
                <div id="tabs-editor" class="tab-pane tabbable tabs-left">
                    <ul class="nav nav-tabs">
                        <?php
                        $x = 0;
                        foreach ($this->profile->editor->form->getFieldsets() as $fieldset) :                            
                            echo '<li><a href="#tabs-editor-' . $fieldset->name . '"><span>' . JText::_('WF_PROFILES_EDITOR_' . strtoupper($fieldset->name)) . '</span></a></li>';
                            $x++;
                        endforeach;
                        ?>
                    </ul>
                    ...
                    <ul class="nav nav-tabs">
                        ...
                        foreach ($this->plugins as $plugin) :
                            if ($plugin->editable && is_file(JPATH_SITE . '/' . $plugin->path . '/' . $plugin->name . '.xml')) :
                                ...
                                echo '<li class="defaultSkin ' . $class . '" data-name="' . $plugin->name . '"><a href="#tabs-plugin-' . $plugin->name . '">' . $icon . '<span class="tabs-label">' . JText::_($plugin->title) . '</span></a></li>';
                            endif;
                        endforeach;
                        ?>
                    </ul>
