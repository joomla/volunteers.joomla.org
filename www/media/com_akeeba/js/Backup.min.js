...
akeeba.Backup.startTimeoutBar=function(a,b){var c=0;akeeba.Backup.timeoutTimer=setInterval(function(){c++;var a=document.querySelector("#response-timer div.text");a&&(a.textContent=akeeba.System.Text._("COM_AKEEBA_BACKUP_TEXT_LASTRESPONSE").replace("%s",c.toFixed(0)))},1E3)};
akeeba.Backup.resetTimeoutBar=function(){Piecon.setOptions({color:"#333333",background:"#e0e0e0",shadow:"#000000",fallback:"force"});try{clearInterval(akeeba.Backup.timeoutTimer)}catch(b){}var a=document.querySelector("#response-timer div.text");a&&(a.textContent=akeeba.System.Text._("COM_AKEEBA_BACKUP_TEXT_LASTRESPONSE").replace("%s","0"))};
...
akeeba.Backup.start=function(){try{console.log("Starting backup"),console.log(data)}catch(e){}if(window.AVGRUN){try{console.warn("AVG Antivirus with Link Checker detected. The backup WILL fail!")}catch(e){}if(!confirm(akeeba.System.Text._("COM_AKEEBA_BACKUP_TEXT_AVGWARNING")))return!1}var a="";try{a=document.getElementById("jpskey").value}catch(e){a=""}var b="";try{b=document.getElementById("angiekey").value}catch(e){b=""}document.getElementById("backup-setup").style.display="none";document.getElementById("backup-progress-pane").style.display=
"block";var c=akeeba.System.getOptions("akeeba.Backup.config_angiekey","");b&&!c&&(document.getElementById("angie-password-warning").style.display="block");c=new Date;akeeba.System.notification.notify(akeeba.System.Text._("COM_AKEEBA_BACKUP_TEXT_BACKUPSTARTED")+" "+c.toLocaleString());akeeba.Backup.renderBackupSteps("");c=akeeba.System.getOptions("akeeba.Backup.maxExecutionTime",14);var d=akeeba.System.getOptions("akeeba.Backup.runtimeBias",75);akeeba.Backup.startTimeoutBar(c,d);a={ajax:"start",description:document.getElementById("backup-description").value,
comment:document.getElementById("comment").value,jpskey:a,angiekey:b};akeeba.System.doAjax(a,akeeba.Backup.onStep,akeeba.Backup.onError,!1);return!1};
akeeba.Backup.onStep=function(a){try{console.log("Running backup step"),console.log(a)}catch(d){}akeeba.Backup.renderBackupSteps(a.Domain);akeeba.Backup.currentDomain=a.Domain;document.querySelector("#backup-percentage div.akeeba-progress-fill").style.width=a.Progress+"%";100<=a.Progress?Piecon.setProgress(99):Piecon.setProgress(a.Progress);document.getElementById("backup-step").textContent=a.Step;document.getElementById("backup-substep").textContent=a.Substep;a.Warnings=a.Warnings||[];if(0<a.Warnings.length){-1==
document.getElementById("backup-percentage").className.indexOf("akeeba-progress--warning")&&(document.getElementById("backup-percentage").className="akeeba-progress--warning");for(var b=0;b<a.Warnings.length;b++){warning=a.Warnings[b];akeeba.System.notification.notify(akeeba.System.Text._("COM_AKEEBA_BACKUP_TEXT_BACKUPWARNING"),warning);var c=document.createElement("div");c.textContent=warning;document.getElementById("warnings-list").appendChild(c)}document.getElementById("backup-warnings-panel").style.display=
"block"}b=a.Error;if(""!=b){try{console.error("Got an error message"),console.log(b)}catch(d){}akeeba.Backup.onError(b)}else if(1==a.HasRun){try{console.log("Backup complete"),console.log(b)}catch(d){}akeeba.Backup.onDone()}else{empty(akeeba.Backup.tag)&&(akeeba.Backup.tag="backend");akeeba.Backup.backupid=a.backupid;akeeba.Backup.resume.retry=0;b=10;a.hasOwnProperty("sleepTime")&&(b=a.sleepTime);try{console.log("Starting tick timer with waitTime = "+b+" msec")}catch(d){}akeeba.Backup.timer(b)}};
...
akeeba.Backup.endWithError=function(a){akeeba.Backup.resetTimeoutBar();try{Piecon.reset()}catch(e){}var b=!1;document.getElementById("backup-progress-pane").style.display="none";document.getElementById("backup-warnings-panel").style.display="none";document.getElementById("retry-panel").style.display="none";var c=akeeba.System.getOptions("akeeba.Backup.URLs.LogURL","")+"&tag="+akeeba.Backup.tag,d=akeeba.System.getOptions("akeeba.Backup.URLs.AliceURL","")+"&log="+akeeba.Backup.tag;akeeba.Backup.backupid&&
(c=c+"."+encodeURIComponent(akeeba.Backup.backupid),d=d+"."+encodeURIComponent(akeeba.Backup.backupid));"finale"==akeeba.Backup.currentDomain&&(b=!0,d+="&autorun=1");document.getElementById("ab-viewlog-error").setAttribute("href",c);document.getElementById("ab-alice-error").setAttribute("href",d);akeeba.System.notification.notify(akeeba.System.Text._("COM_AKEEBA_BACKUP_TEXT_BACKUPFAILED"),a);akeeba.System.doAjax({ajax:"pushFail",tag:akeeba.Backup.tag,backupid:akeeba.Backup.backupid,errorMessage:a},
function(a){});document.getElementById("backup-error-message").textContent=a;document.getElementById("error-panel").style.display="block";b&&setTimeout(function(){window.location=d},500)};
akeeba.Backup.onDone=function(){var a=new Date;akeeba.System.notification.notify(akeeba.System.Text._("COM_AKEEBA_BACKUP_TEXT_BACKUPFINISHED")+" "+a.toLocaleString());akeeba.Backup.resetTimeoutBar();try{Piecon.reset()}catch(c){}document.getElementById("backup-progress-pane").style.display="none";document.getElementById("backup-complete").style.display="block";document.getElementById("backup-warnings-panel").style.width="100%";a=akeeba.System.getOptions("akeeba.Backup.URLs.LogURL","");var b=a+"&tag="+
akeeba.Backup.tag;akeeba.Backup.tag?akeeba.Backup.backupid&&(b=b+"."+encodeURIComponent(akeeba.Backup.backupid)):b=a+"&latest=1";try{document.getElementById("ab-viewlog-success").setAttribute("href",b)}catch(c){}a=akeeba.System.getOptions("akeeba.Backup.returnUrl",akeeba.Backup.returnUrl);""!=a&&(window.location=a)};
...
akeeba.Backup.restoreCurrentOptions=function(){var a=document.getElementById("backup-description");null!==a&&(a.value=akeeba.System.getOptions("akeeba.Backup.currentDescription",""));a=document.getElementById("angiekey");null!==a&&(a.value="ThisIsADummyStringToWorkAroundChrome",a.value=akeeba.System.getOptions("akeeba.Backup.config_angiekey",""));a=document.getElementById("jpskey");null!==a&&(a.value="ThisIsADummyStringToWorkAroundChrome",a.value=akeeba.System.getOptions("akeeba.Backup.jpsKey",""));
a=document.getElementById("comment");null!==a&&(a.value="ThisIsADummyStringToWorkAroundChrome",a.value=akeeba.System.getOptions("akeeba.Backup.currentComment",""))};akeeba.Backup.flipProfile=function(){document.getElementById("flipDescription").value=document.getElementById("backup-description").value;document.getElementById("flipComment").value=document.getElementById("comment").value;document.forms.flipForm.submit()};
akeeba.System.documentReady(function(){akeeba.System.notification.askPermission();akeeba.System.addEventListener("comAkeebaBackupProfileDropdown","change",akeeba.Backup.flipProfile);akeeba.System.addEventListener("comAkeebaBackupFlipProfile",function(){akeeba.Backup.flipProfile();return!1});akeeba.System.addEventListener("comAkeebaBackupCancelResume",akeeba.Backup.cancelResume);akeeba.System.addEventListener("comAkeebaBackupResumeBackup",akeeba.Backup.resumeBackup);akeeba.System.getOptions("akeeba.Backup.autostart")?
akeeba.Backup.start():(akeeba.System.addEventListener("backup-start","click",akeeba.Backup.start),akeeba.System.addEventListener("backup-default","click",akeeba.Backup.restoreDefaultOptions),setTimeout(akeeba.Backup.restoreCurrentOptions,500))}); //# sourceMappingURL=Backup.min.map
