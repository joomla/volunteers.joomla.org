...
akeeba.Restore.doAjax=function(a,c,e){json=JSON.stringify(a);a={json:json};var b=akeeba.System.getOptions("akeeba.Restore.password","");0<b.length&&(a.password=b);b=(new Date).getTime()/1E3;var g=parseInt(b,10);a._cacheBustingJunk=Math.round(1E3*(b-g));a={type:"POST",url:akeeba.System.getOptions("akeeba.Restore.ajaxURL",""),cache:!1,data:a,timeout:6E5,success:function(a){var f=a.indexOf("###");if(-1==f)a="Invalid AJAX data: "+a,null==e?null!=akeeba.Restore.errorCallback?akeeba.Restore.errorCallback(a):
akeeba.Restore.errorCallbackDefault(a):e(a);else{0!=f&&(a.substr(0,f),a=a.substr(f));a=a.substr(3);f=a.lastIndexOf("###");a=a.substr(0,f);try{var b=JSON.parse(a)}catch(d){errorMessage=d.message+"\n<br/>\n<pre>\n"+a+"\n</pre>";null==e?null!=akeeba.Restore.errorCallback?akeeba.Restore.errorCallback(errorMessage):akeeba.Restore.errorCallbackDefault(errorMessage):e(errorMessage);return}c(b)}},error:function(a,c,b){b=a.responseText?a.responseText:"";var d="<strong>AJAX Loading Error</strong><br/>HTTP Status: "+
a.status+" ("+a.statusText+")<br/>";d=d+"Internal status: "+c+"<br/>XHR ReadyState: "+a.readyState+"<br/>";d=d+"Raw server response:<br/>"+akeeba.System.sanitizeErrorMessage(b);null==e?null!=akeeba.Restore.errorCallback?akeeba.Restore.errorCallback(d):akeeba.Restore.errorCallbackDefault(d):e(d)}};akeeba.Ajax.ajax(a)};
akeeba.Restore.startTimeoutBar=function(a,c){akeeba.Restore.resetTimeoutBar();akeeba.Restore.timer=setInterval(function(){akeeba.Restore.lastResponseSeconds++;var a=akeeba.System.Text._("COM_AKEEBA_BACKUP_TEXT_LASTRESPONSE").replace("%s",akeeba.Restore.lastResponseSeconds.toFixed(0));try{document.getElementById("response-timer").querySelector("div.text").textContent=a}catch(b){}},1E3)};
akeeba.Restore.resetTimeoutBar=function(){akeeba.Restore.lastResponseSeconds=0;if(0!=akeeba.Restore.timer){clearInterval(akeeba.Restore.timer);akeeba.Restore.timer=0;var a=document.getElementById("response-timer").querySelector("div.text"),c=akeeba.System.Text._("COM_AKEEBA_BACKUP_TEXT_LASTRESPONSE").replace("%s","0");try{a.textContent=c}catch(e){}}};
...
akeeba.Restore.statistics.inbytes,document.getElementById("extbytesout").textContent=akeeba.Restore.statistics.outbytes,document.getElementById("extfiles").textContent=akeeba.Restore.statistics.files}catch(c){}post={task:"stepRestore",factory:a.factory};akeeba.Restore.startTimeoutBar(5E3,80);akeeba.Restore.doAjax(post,function(a){akeeba.Restore.step(a)})}};
akeeba.Restore.finalize=function(a){a.preventDefault();a={task:"finalizeRestore",factory:akeeba.Restore.factory};akeeba.Restore.startTimeoutBar(5E3,80);akeeba.Restore.doAjax(a,function(a){akeeba.Restore.finished(a)});return!1};akeeba.Restore.finished=function(){window.location=akeeba.System.getOptions("akeeba.Restore.mainURL",window.location)};
