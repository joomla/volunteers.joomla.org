...
akeeba.Restore.doAjax=function(a,c,d){json=JSON.stringify(a);a={json};var g=akeeba.System.getOptions("akeeba.Restore.password","");0<g.length&&(a.password=g);g=(new Date).getTime()/1E3;var k=parseInt(g,10);a._cacheBustingJunk=Math.round(1E3*(g-k));a={type:"POST",url:akeeba.System.getOptions("akeeba.Restore.ajaxURL",""),cache:!1,data:a,timeout:6E5,success:function(b){var f=b.indexOf("###");if(-1==f)b="Invalid AJAX data: "+b,null==d?null!=akeeba.Restore.errorCallback?akeeba.Restore.errorCallback(b):
