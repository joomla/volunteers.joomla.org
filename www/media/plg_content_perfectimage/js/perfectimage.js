        ...
        if (url) {
            inputImage.change(function () {
                ...
                if (!image.data('cropper')) {
                    var ratio = $('#' + id + ' .perfect-image-ratio').val(),
                        ratioSplit = ratio.split('/');
                ...
                if (files && files.length) {
                    ...
                    var imageMaxSize = $('.perfect-image-maxsize');
                    var dimensionSizes = $('.perfect-image-dimensionsize');
                    if (file.size <= imageMaxSize.val() || imageMaxSize.val() == 0)
                    {
                        if (/^image\/\w+$/.test(file.type)) {
                            bloburl = URL.createObjectURL(file);

                            var $tmpImageHolder = $('#temp-image-holder');
                            $tmpImageHolder.off().load(function() {
                                // now check for dimension size
                                if (this.naturalHeight > dimensionSizes.val() || this.naturalWidth > dimensionSizes.val()) {
                                    window.alert($('.perfect-image-maxsize-message').val());
                                    inputImage.val("");

                                    $(image).attr('src', '');

                                    image.cropper('reset').cropper('clear').cropper('destroy');
                            ...
                            $tmpImageHolder.attr('src', bloburl);
                        } else {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.message) {
                    ...
                    console.log('Image uploaded: ' + response.data[0].relative);
                    var image = response.data[0];

                    $('input#' + id + '').val(image.relative).trigger('change');
                    $('#' + id + ' .perfect-image-preview').html('<img src="' + image.absolute + '" />');
