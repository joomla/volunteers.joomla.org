...
class plgAuthenticationAwoELogin extends JPlugin {
	...
	public function onUserAuthenticate($credentials, $options, &$response) {
		...
		if ($result) {
			...
			if(method_exists('JUserHelper','verifyPassword')) $match = JUserHelper::verifyPassword($credentials['password'], $result->password, $result->id);
			else {
				if (substr($result->password, 0, 4) == '$2y$') {
					...
					if (JCrypt::hasStrongPasswordSupport()) $match = password_verify($credentials['password'], $password60);
				}
				elseif (substr($result->password, 0, 8) == '{SHA256}') {
					...
					if ($result->password == $testcrypt) $match = true;
				}
				else {
					// Check the password
					$parts	= explode(':', $result->password);
		...
		if ($response->status == JAuthentication::STATUS_SUCCESS && version_compare( JVERSION, '3.2.0', 'ge' )) {
