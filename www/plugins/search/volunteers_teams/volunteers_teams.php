...
class plgSearchVolunteers_teams extends JPlugin
{
	/**
	...
	{
		static $areas = array(
			'teams' => 'COM_VOLUNTEERS_TITLE_TEAMS'
		);
	...
	public function onContentSearch($text, $phrase = '', $ordering = '', $areas = null)
	{
		...
		$query
			->select('id, title, created, description AS text, \'' . JText::_('COM_VOLUNTEERS_TITLE_TEAMS') . '\' AS section, \'1\' AS browsernav')
			->from($db->qn('#__volunteers_teams'))
		...
		{
			$item->href = JRoute::_('index.php?option=com_volunteers&view=team&id=' . $item->id);
		}
