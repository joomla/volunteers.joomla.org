...
class Mysqldump
{
    ...
    // gweMod
    public $foreignConstraints = array();
    ...
    private function getTableStructure($tableName)
    {
        if (!$this->dumpSettings['no-create-info']) {
            ...
            // GWE MOD
            // You may need to rename foreign keys to prevent name re-use error - store this data for later
            $fkcheck = "SELECT * FROM information_schema.key_column_usage WHERE REFERENCED_TABLE_NAME <> '' AND  TABLE_SCHEMA = '" . $this->dbName . "' AND TABLE_NAME = '$tableName'";
            $fkcheckData = $this->dbHandler->query($fkcheck);
            $fkcheckData->setFetchMode(PDO::FETCH_ASSOC);
            foreach ($fkcheckData as $fkcheckDatum)
            {
                if (isset($fkcheckDatum['CONSTRAINT_NAME']) && strpos($fkcheckDatum['CONSTRAINT_NAME'], $this->compressManager::$oldPrefix) === false)
                {
                    $this->foreignConstraints = array_merge( $this->foreignConstraints, array($fkcheckDatum) );
                }
            }
            // end GWE MOD
